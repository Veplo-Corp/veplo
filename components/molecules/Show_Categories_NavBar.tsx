...
const Show_Categories_NavBar: React.FC<{ gender: string, closeCategory: any }> = ({ gender, closeCategory }) => {
    ...
    const handleClickCategory = (categorySelected: string) => {                
        ...
        const categoryForUrl = Object.values(categories)[indexArray].abbigliamento.find(category => category.name === categorySelected)?.url
        console.log(categoryForUrl);
        
        if (!categoryForUrl) return
        const categorySelectedUrl = createUrlSchema([categoryForUrl])
